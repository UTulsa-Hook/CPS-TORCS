...
void Driver::initTrack(tTrack* t, void *carHandle, void **carParmHandle, tSituation *s)
{
	...
	switch (s->_raceType) {
		case RM_TYPE_PRACTICE:
			snprintf(buffer, BUFSIZE, "drivers/jgf/%d/practice/%s", INDEX, trackname);
			break;
		case RM_TYPE_QUALIF:
			snprintf(buffer, BUFSIZE, "drivers/jgf/%d/qualifying/%s", INDEX, trackname);
			break;
		case RM_TYPE_RACE:
			snprintf(buffer, BUFSIZE, "drivers/jgf/%d/race/%s", INDEX, trackname);
			break;
		default:
			break;
	}
	...
	*carParmHandle = GfParmReadFile(buffer, GFPARM_RMODE_STD);
	if (*carParmHandle == NULL) {
		snprintf(buffer, BUFSIZE, "drivers/jgf/%d/default.xml", INDEX);
		*carParmHandle = GfParmReadFile(buffer, GFPARM_RMODE_STD);
    }
	...
	MU_FACTOR = GfParmGetNum(*carParmHandle, JGF_SECT_PRIV, JGF_ATT_MUFACTOR, (char*)NULL, 0.69f);
}
...
void Driver::newRace(tCarElt* car, tSituation *s)
{
	...
	// Set team mate.
	const char *teammate = GfParmGetStr(car->_carHandle, JGF_SECT_PRIV, JGF_ATT_TEAMMATE, NULL);
	if (teammate != NULL) {
